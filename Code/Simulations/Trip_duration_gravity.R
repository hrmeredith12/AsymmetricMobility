...
#  relationship between asymmetry and trip duration
asym <- test.ss[["A"]]
asym.long <- melt(asym, value.name = "asymmetry", varnames = c('origin', 'destination'))
lambda.long <- melt(lambda_mixed, value.name = "lambda", varnames = c('origin', 'destination'))
asym.lambda <- left_join(asym.long, lambda.long, by = c('origin', 'destination'))

ggplot(asym.lambda, aes(1/lambda, asymmetry))+
  geom_point()+
  labs(x = "Mean trip duration (days)", y = "Asymmetric portion")


ggplot(asym.long, aes(asymmetry))+
  geom_histogram()+
  facet_wrap(~origin)

asym.long.no.stay <- subset(asym.long, origin != destination)
ggplot(asym.long.no.stay, aes(asymmetry))+
  geom_histogram()+
  facet_wrap(~origin)



## Looking at Gini coefficient to determine distribution of asymmetry across destinations
## Compares Gini coefficient of asymmetry for all trips (abs value), 
## trips with only neg asymmetry values, and trips with only positive asymmetry values 

## Consider all trips to/from all locations
all.neg = subset(asym.long, asymmetry<=0)
all.pos = subset(asym.long, asymmetry>=0)
Gini(abs(asym.long$asymmetry))
Gini(abs(all.neg$asymmetry))
Gini(abs(all.pos$asymmetry))
ggplot(asym.long, aes(asymmetry))+
  geom_histogram()

## Consider all trips originating at location 1
loc1 <- subset(asym.long, origin == 1)
loc1.pos <- subset(loc1, asymmetry>=0)
loc1.neg <- subset(loc1, asymmetry<=0)
Gini(abs(loc1$asymmetry))
Gini(abs(loc1.pos$asymmetry))
Gini(abs(loc1.neg$asymmetry))

loc2 <- subset(asym.long, origin == 2)
loc2.pos <- subset(loc2, asymmetry>=0)
loc2.neg <- subset(loc2, asymmetry<=0)
Gini(abs(loc2$asymmetry))
Gini(abs(loc2.pos$asymmetry))
Gini(abs(loc2.neg$asymmetry))

loc3 <- subset(asym.long, origin == 3)
loc3.pos <- subset(loc3, asymmetry>=0)
loc3.neg <- subset(loc3, asymmetry<=0)
Gini(abs(loc3$asymmetry))
Gini(abs(loc3.pos$asymmetry))
Gini(abs(loc3.neg$asymmetry))

loc4 <- subset(asym.long, origin == 4)
loc4.pos <- subset(loc4, asymmetry>=0)
loc4.neg <- subset(loc4, asymmetry<=0)
Gini(abs(loc4$asymmetry))
Gini(abs(loc4.pos$asymmetry))
Gini(abs(loc4.neg$asymmetry))

loc6 <- subset(asym.long, origin == 6)
loc6.pos <- subset(loc6, asymmetry>=0)
loc6.neg <- subset(loc6, asymmetry<=0)
Gini(abs(loc6$asymmetry))
Gini(abs(loc6.pos$asymmetry))
Gini(abs(loc6.neg$asymmetry))

loc8 <- subset(asym.long, origin == 8)
loc8.pos <- subset(loc8, asymmetry>=0)
loc8.neg <- subset(loc8, asymmetry<=0)
Gini(abs(loc8$asymmetry))
Gini(abs(loc8.pos$asymmetry))
Gini(abs(loc8.neg$asymmetry))

ggplot(subset(asym.long, origin %in% c(1,2,3,6)), aes(asymmetry))+
  geom_histogram()+
